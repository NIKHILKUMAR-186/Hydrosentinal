...
export const Simulator = ({ onPosted, onRunningChange }: { onPosted: () => void; onRunningChange?: (running: boolean) => void }) => {
  ...
  return (
    ...
    >
      <Activity className={`h-4 w-4 ${running ? "text-green-400 animate-pulse" : "text-gray-400"}`} />
      <span className="font-semibold">{running ? "▶ Running" : "⏸ Paused"}</span>
    </button>
  );
};
